...
from user_api_contact import check_user, get_user_inf, add_contact_to_user, get_contact_info
import os
...
async def crate(contact: SchemaContact):
    try:
        ...
        db.session.commit()
        created_contact = get_contact_info(c_name=contact.name, telephone=contact.telephone, owner=contact.owner)
...
async def add_user(request: Request):
    ...
    try:
        ...
        if req_json["contact"] not in user_info["contact_id"]:
            contact = db.session.query(ModelContract).filter_by(id=req_json["contact"]).one()
